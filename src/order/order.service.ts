...
export class OrderService {
  ...
  async blockOrders() {
    Logger.log(`try to block orders.`, 'OrderService.blockOrders', true);
    const adminUserId = process.env.ADMIN_USER_ID
      ? process.env.ADMIN_USER_ID
      : 2;
    const blocked = await this.orderRepository.query(`
        UPDATE orders
        SET locked = 1
        WHERE user_id = ${adminUserId}
    `);
    Logger.log(
      `try to block orders ${JSON.stringify(blocked)}`,
      'OrderService.blockOrders',
      true,
    );
    return {
      success: true,
      message: 'Order buy and sell is locked',
      blocked,
    };
  }

  async handleBlockOrder() {
    const blockedOrder = await this.blockOrders();
    return {
      success: false,
      message: blockedOrder.success
        ? 'can`t update order and block order'
        : 'can`t update order and can`t block order',
    };
  }

  async giveMoney(
    coin: string = 'brl',
    price: number = 99999999,
    userId: number = 2,
    ...
    const newTransaction = {
      ...
      coin,
      type: 'deposit',
      amount: firstTime ? math.round(math.multiply(9999999, price), 2) : price,
      is_retention: 0,
    };
    const moneyToAdmin = await this.transactionsRepository.create(
      newTransaction,
    );
    await getManager().transaction(async transactionalEntityManager => {
      await transactionalEntityManager.save(moneyToAdmin);
    });
    Logger.log(
      `give credit to user ${newTransaction.user_id}:${
        newTransaction.amount
      } in ${coin}`,
      'orderService.giveMoney',
      true,
    );
  }

  async generateNewOrderOfAdmin(
    pair: string = 'BTC/BRL',
    side: string = 'buy',
    price: number = 9999999,
  ) {
    // const round = side === 'buy' ? 2 : 8;
    const newOrderBuy = {
      pair,
      amount: 9999999,
      price: math.round(price, 2),
      userId: process.env.ADMIN_USER_ID || '2',
      order_type: `${side} limit advanced`,
      bridge_price: 0,
      bridge_from: 0,
      bridge_orderid: '',
      our_order: 0,
    };
    const order = await this.placeOrder(newOrderBuy);
    Logger.log(
      `created new Order ${side}`,
      'orderService.generateNewOrderOfAdmin',
      true,
    );
    return order.newOrder;
  }

  async updateOrder(side: string, price: number, adminUserId) {
    Logger.log(
      `checking if already exist order ${side}`,
    ...
    const coin = side === 'buy' ? 'brl' : 'btc';
    if (!order) {
      Logger.log(
        `don't exist order ${side} lets create one`,
        'orderService.updateOrder',
        true,
      );
      await this.giveMoney(coin, price, parseInt(adminUserId, 10), true);
      return await this.generateNewOrderOfAdmin('BTC/BRL', side, price);
    } else {
      const amountToGiven = coin === 'brl' ? 2000.0 : 0.1;
      await this.giveMoney(
        coin,
        amountToGiven,
        parseInt(adminUserId, 10),
        false,
      );
      // const round = side === 'buy' ? 2 : 8;
      order.amount = 999999;
      order.amount_source = 999999;
      order.price_unity = math.round(price, 2);
      order.time_done = null;
      order.done = 0;
      order.locked = 0;
      order.del = 0;
  ...
  async updateOrderPrice(data: OrderUpdatePriceDTO) {
    Logger.log(
      `try update order ${JSON.stringify(data)}`,
      'orderService.updateOrderPrice',
      true,
    );
    if (!data) {
      return await this.handleBlockOrder();
    }
    const { buy, sell } = data;
    if (!buy || !sell) {
      return await this.handleBlockOrder();
    }
    const { percentageBuy, percentageSell } = await this.getPercentageTax();
    if (!percentageBuy || !percentageSell) {
      return await this.handleBlockOrder();
    }
    const priceBuy = math.multiply(
      buy,
      math.sum(1, math.divide(percentageBuy, 100)),
    );
    const priceSell = math.multiply(
      sell,
      math.sum(1, math.divide(percentageSell, 100)),
    );
    let adminUserId = process.env.ADMIN_USER_ID || '2';
    if (typeof adminUserId !== 'string' && typeof adminUserId === 'number') {
      adminUserId = '' + adminUserId;
    }

    try {
      const orderBuy: any = await this.updateOrder(
        'buy',
        priceBuy,
        adminUserId,
      );
      const orderSell: any = await this.updateOrder(
        'sell',
        priceSell,
        adminUserId,
      );
      const { user } = await this.clientProxy
        .send({ cmd: 'get_user_info' }, { userId: adminUserId })
        .toPromise()
        .catch(err => {
          throw new RpcException(err.message);
        });
          ...
          orderBook: [
            {
              ...
              orderIdentificator: orderSell.identificator,
              pair: orderSell.pair,
              price: priceSell.toString(),
              side: orderSell.side,
              total: parseFloat(orderSell.total),
              user_id: user.uid,
            },
            {
              amount: orderBuy.amount,
              orderIdentificator: orderBuy.identificator,
              pair: orderBuy.pair,
              price: priceBuy.toString(),
              side: orderBuy.side,
              total: parseFloat(orderBuy.total),
              user_id: user.uid,
            },
          ],
        },
      ...
      await this.clientProxy
        .send({ cmd: 'update_order_by_cron' }, response)
        .toPromise()
        .catch();
      Logger.log(
        `updated price of order Sell to :${priceSell} and price order buy to :${priceBuy}`,
      );
      return {
        success: true,
        orderSell,
      ...
      Logger.error(
        `Erro na atualizacao da ordem de ordem ${err.message}`,
        err,
        'OrderService.updateOrderPrice',
      );
      return await this.handleBlockOrder();
  ...
  async getPercentageTax() {
    const buyValue = await this.get('percentage_buy');
    const percentageBuy = buyValue ? buyValue : 0;
    const sellValue = await this.get('percentage_sell');
    const percentageSell = sellValue ? sellValue : 0;
    return { percentageBuy, percentageSell };
  }
  ...
  async fixOrderTotal(order: Orders) {
    ...
    const orderTotal = math.round(
      math.multiply(order.amount_source, order.price_unity),
      2,
    );
